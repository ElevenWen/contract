...
function regularJudgement(str, val) {
  ...
  }else if (str == "companyName") {//公司名称
    var reg = /^[\(\)\（\）\u4e00-\u9fa5\w]{2,}$/.test(val); 
    if (reg && val != undefined && val.trim().length != 0) {
      return true;
    } else {
      Vue.$vux.toast.text('企业名称有误请重新输入', 'middle')
      return false;
    } //电话判断
  }  
...
export function getStyle(dom,property){
  ...
  if(window.currentStyle){
    return dom.currentStyle[property].replace(/[a-z]/ig,'');
  }else{
    //chrome
    return window.getComputedStyle(dom , null)[property].replace(/[a-z]/ig,'');
  }
}
