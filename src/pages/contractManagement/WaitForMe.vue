...
<template   >  
<div> 
      <div  id="WaitForMe"  class="content"  >   

        <div v-show="$attrs.pulldowntext"  class="defaultRefreshStyle"  :class="{nodataStyle:_WaitForMeData.recordCount == 0}">下拉刷新</div> 
        <div  class="dataWrap" v-if="_WaitForMeData.recordCount != 0">
             <div  class="pulldowntext" v-if='false'>{{pulldowntext}}</div>
              <div class="data">
                <div v-for="(items,index) in _WaitForMeData.list" :key="index" class="iteratorItems"> 
                        <div >
        ...
        <div class="pullUptext" v-show="_WaitForMeData.recordCount != 0 && $attrs.pullUptext">没有更多了...</div>  
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("contractStateManagement", ["_WaitForMeData"])
  },
  created() { 
  },
  mounted() {
  ...
  methods: {
    aaa() { 
      this.$emit("childrenEmitProps", "true");
    },
