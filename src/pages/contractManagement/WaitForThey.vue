...
<template   >  
<div> 
    <div id="WaitForThey" class="content"  >  
      <div v-show="$attrs.pulldowntext"  class=" defaultRefreshStyle ">下拉刷新</div> 
      <div  class="dataWrap" v-if="_WaitForTheyData.recordCount != 0">
            <div  class="pulldowntext" v-if='false'>{{pulldowntext}}</div>
            <div class="data">
              <div v-for="(items,index) in _WaitForTheyData.list" :key="index" class="iteratorItems"> 
                        <div >
              ...
              <div class="pulldowntext" v-show="hasData=='没有更多了'">   {{_WaitForTheyData.recordCount}}  ====== {{$attrs.pullUptext}}</div> 
               <div></div>
            </div> 
      </div>
      ...
      <div class="pullUptext" v-show="_WaitForTheyData.recordCount != 0 && $attrs.pullUptext">没有更多了...</div>  
    </div> 
...
export default {
  ...
  data() {
   ...
   computed: {  
    ...
    ...mapGetters("contractStateManagement", ["_WaitForTheyData"]), 
  },
  created() {
